...
export const fetchPositionProfileUpdateDetail = (id) => {
    ...
    return (dispatch) => {
        return axios.get(
            ...
            var skillDetail = res.data.resultObj
            if (skillDetail !== null) {
                if (skillDetail.hardSkills.length > 0) {
                    skillDetail.hardSkills.forEach(element => {
                        element.certiList = []
                        var url = `${API_URL}/Certification/getCertifications/${element.skillID}`
                        axios.get(
                            url,
                            { headers: { "Authorization": `Bearer ${localStorage.getItem('token').replace(/"/g, "")}` } }
                        ).then(res1 => {
                            var listCerti = res1.data.resultObj
                            element.certiList = listCerti
                        })
                    });
                }
                console.log('skillDetail',skillDetail)
                dispatch(fetchPositionProfileUpdateDetailSuccess(skillDetail))
            }
